...
4. Create a D1 database with `pnpm wrangler d1 create sink`, then update the `d1_databases` ID in `wrangler.jsonc` with the returned database ID.
5. Apply the D1 schema with `pnpm db:migrate:remote`.
6. (_Optional_) For OpenGraph image upload, create an [R2 bucket](https://developers.cloudflare.com/r2/) named `sink` (or run `wrangler r2 bucket create sink`). If you don't need this feature, comment out the `r2_buckets` section in `wrangler.jsonc`.
7. Create a project in [Cloudflare Workers](https://developers.cloudflare.com/workers/).
8. Select the `Sink` repository and use the following build and deploy commands:
   - **Build command**: `pnpm run build` or `npm run build`
   - **Deploy command**: `pnpm deploy:worker`

9. Save and deploy the project. The deployment command applies pending D1 migrations before publishing the Worker.
...
11. Enable Analytics Engine. In **Workers & Pages**, go to **Account details** in the right panel, locate **Analytics Engine**, and click **Set up** to enable the free tier. Name them `sink` and `ANALYTICS`, or else overwrite it with `NUXT_DATASET` and update your `wrangler.jsonc` accordingly.
12. Redeploy the project.
13. To update your code, refer to the official GitHub documentation: [Syncing a fork branch from the web UI](https://docs.github.com/pull-requests/collaborating-on-github/syncing-a-fork 'GitHub: Syncing a fork').
